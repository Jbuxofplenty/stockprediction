...
class PredictionMachine:
    ...
    """
    Helper function to train models in order to compare to distributed algorithm
    """
    def train_models(self, keys=None):
        total_points = 500
        if keys is None:
            keys = self.model_db.keys()
        for key in keys:
            ...
            # Store the model with the appended serial_number
            if self.model_db[key]['type'] == 'Linear Regressor':
                fname_model = "trash/lr_regression_" + str(serial_num) + "_" + self.table + ".pkl"
                lr.dump(fname_model)
            elif self.model_db[key]['type'] == 'MLP Regressor':
                fname_model = "trash/lr_regression_" + str(serial_num) + "_" + self.table + ".pkl"
                mlpr.dump(fname_model)
    ...
    def update_actual(self):
        ...
        for db_time in formatted_data.keys():
            ...
            self.df.at(db_time, 'actual', formatted_data[db_time])
    ...
    """
    def load_symbols_txt(self, fname='symbols'):
        with open('pickled_files/symbols/' + fname + '.txt', 'r') as f:
            self.symbols = [l.strip().lower() for l in f]
            return self.symbols
